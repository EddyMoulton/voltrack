export class StockChangeViewModel {
  ...
  sumarise(stockChanges: StockChangeViewModel[]) {
    this.value = stockChanges
      .map((change: StockChangeViewModel) => change.value)
      .reduce((total, value) => total + value);
    this.scaledAnnualReturn = stockChanges
      .map((change: StockChangeViewModel) => change.scaledAnnualReturn)
      .reduce((total, value) => (total || 0) + (value || 0));
  }
}
